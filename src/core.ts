...
// GNews: search
export async function gnewsSearch(q: string, opts?: { lang?: string; max?: number }) {
  const effect = pipe(
    get("/api/gnews/search", { schema: GNewsResponseSchema }, { q, lang: opts?.lang ?? "en", max: opts?.max ?? 5 }),
...
export async function gnewsTopHeadlines(opts?: {
  lang?: string; country?: string; category?: "technology" | "general" | "world" | "nation" | "business" | "entertainment" | "sports" | "science" | "health"; max?: number;
}) {
  const effect = pipe(
    get("/api/gnews/top-headlines", { schema: GNewsResponseSchema }, opts ?? {}),
    Effect.provide(FetchHttpClient.layer)
  );
...
export async function googleSearch(q: string) {
  const effect = pipe(
    get("/api/google/cse", { schema: SearchRecommendationSchema }, { q, num: 10 }),
    Effect.provide(FetchHttpClient.layer)
  );
  const res = await Effect.runPromise(effect);
...
export async function youtubeSearch(q: string, pageToken?: string) {
  const effect = pipe(
    get("/api/youtube/search", { schema: YoutubeSearchResponseSchema }, { q, pageToken, maxResults: 10 }),
    Effect.provide(FetchHttpClient.layer)
  );
  const res = await Effect.runPromise(effect);
